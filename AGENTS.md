...
- **独立编译契约与全平台交叉编译**：
  - **契约一致性**：在开发态执行（`ac run`）与构建后的独立可执行文件中，`ActionContext` 的语义、配置优先级、状态持久化与输出 JSON Envelope 格式必须保持严格一致。
  - **全平台支持**：`ac build` 与 `ac export skill --standalone` 支持通过 `-t, --target <target>` 交叉编译为 `linux-x64`、`linux-arm64`、`darwin-x64`、`darwin-arm64`、`windows-x64` 等全平台独立可执行文件，目标主机零运行时依赖开箱即用。
...
- **依赖与 Link 规范**：
  ...
  - **双 Link 区分**：`ac link` 是 ActionDock 全局路由表注册（跨目录调度）；`bun link` 是依赖解析（填充 `node_modules`）。两者职责独立，开发态通常都需要执行。
  - **工作区与子项目自动感知**：`ac link [path]` 支持单包与工作区容器目录。在包含多个子包的目录执行时，会自动注册工作区并扫描子包；后续在工作区内新增子包无需重新 link，全局路由与 `ac info` 自动动态感知。
- **能力发现与模糊探索规范**：
  ...
  - **树形层级概览**：执行 `ac info --tree` 可查看当前注册的工作区结构与包含的子包清单及健康状态。
  - **决议机制**：唯一命中时自动展开该包完整详情与配置依赖；多项命中时输出匹配包清单；未命中时默认降级展示全量已注册包。
- **环境诊断与健康体检**：
  - **诊断入口**：在遇到环境异常、软链失效或配置缺失时，执行 `ac doctor`（或 `ac doctor --json`）进行全套运行时、SQLite 读写、注册表健康度与项目依赖检测，并依提示执行修复。
