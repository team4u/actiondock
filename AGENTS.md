...
- **通用设计与协作原则**：
  - **领域内核与外部实现解耦**：核心领域保持环境与平台中立，不依赖具体外部实现；外部能力与平台差异统一通过抽象接口按需注入。
  - **单一职责与拒绝代码膨胀**：模块保持职责单一，避免上帝对象与超大单文件；复杂服务按领域拆分路由控制器，复合命令原子化解耦。
  - **单一事实源与杜绝重复实现**：严禁跨模块拷贝相同逻辑；测试代码直接复用生产领域实现，通用工具统一下沉，保持单一事实来源。
  - **非阻塞异步优先**：严禁在异步服务或执行链路中调用同步阻塞操作；耗时命令必须支持超时终止与协作式取消。
  - **防御性设计与错误透明**：严禁静默吞没异常，错误必须类型化并保留上下文；对外交互严格执行边界校验，保持退出码与资源释放规范一致。
  - **响应式事件驱动协作**：异步流程或多代理协作中严禁周期性轮询；必须基于事件通知与响应式自动唤醒机制推进。
- **文档与输出风格规范**：
