...
- **独立编译契约原则**：
  - 在开发态执行（`ac run`）与构建后的独立可执行文件中，`ActionContext` 的语义、配置优先级、状态持久化与输出 JSON Envelope 格式必须保持严格一致。
- **外部 CLI 执行规范**：
  ...
  - **防管道死锁与后台守护进程**：
    ...
    - **拉起守护进程的命令（如 `agent-browser open`）**：严禁使用带管道的同步等待（常驻守护进程继承标准错误与输出句柄导致管道永不结束挂死），必须使用 `spawnDetached`（标准输入输出全部忽略异步启动、等待前端退出错开冷启动、轮询探测确认就绪）；守护进程存活后的后续操作再使用常规 `execCli`。
