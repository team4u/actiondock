...
`project-knowledge-maintainer` 的出发点不是让 AI 多写几页文档，而是把知识维护过程本身约束住。项目事实散在代码、配置、DDL、测试、日志和现有文档里；如果不先约束证据来源，模型很容易把一次性上下文当成长期事实。

所以这个 Skill 的第一层约束是证据边界：当前仓库文件优先，证据不足就跳过、失败或写进报告，不为了结构完整伪造内容。第二层约束是角色边界：Chief 只做全局分诊，Planner 只形成目标文件任务，Worker 只收敛一个目标文件。拆成三层不是为了制造复杂度，而是为了单一职责和上下文控制；每一层只接收自己需要的信息，幻觉空间也会变小。
...
flowchart TD
  ...
  B --> C[Chief<br/>全局分诊]
  C --> D[Planner<br/>目标文件任务]
  D --> E[Worker<br/>单文件收敛]
  E --> F[docs/ 与 ACTIONDOCK.md]
  F --> G[验证 / 报告]
  B -->|证据不足或路径不安全| H[跳过 / 失败 / 记录]
```
