...
ActionDock 提供了灵活且安全的配置解析机制，并支持 **Demo 降级机制**，使用者无需修改任何源码即可体验与注入配置。

---

## 开箱即用 Demo 模式 vs 真实凭证

官方示例（如 `github-tools`）设计了优雅的 Mock 降级策略：
- **未配置 Token 时**：自动进入 Demo 模式并输出警告日志，返回标准结构化的模拟数据（如示例 PR 列表），方便消费者快速验证调用链路。
