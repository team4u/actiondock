...
当你需要将 Action Package 部署为长期运行的微服务，供远程 AI Agent、Webhook 或前端业务系统调用时，可以使用 `ac serve` 启动 HTTP 调度服务。
...
### B. 异步执行长任务 (POST `/actions/:id?async=true`)
