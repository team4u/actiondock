...
│ 目标场景与宿主          │ 推荐接入路径                  │ 对应指引                           │
├─────────────────────────┼───────────────────────────────┼────────────────────────────────────┤
│ AI 智能体自主消费       │ Agent Skill 技能包            │ Agent Skill 使用指南               │
│ IDE 与开发工具接入      │ MCP 协议服务 (STDIO / HTTP)   │ 开发工具 MCP 接入                  │
│ 服务器与容器生产部署    │ Node 交付目录 / HTTP 微服务   │ Node 交付产物运行 / HTTP 微服务    │
│ ActionDock 业务工程集成 │ 工程依赖安装 (ad add)         │ 本文「工程依赖消费」小节           │
└─────────────────────────┴───────────────────────────────┴────────────────────────────────────┘
```
