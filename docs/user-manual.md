...
1. 打开管理台，进入「脚本库」页面
2. 找到 `hello-groovy` 脚本
3. 点击「运行」按钮
4. 在输入表单中填入 `name: alice`
5. 点击执行，查看结果
...
1. 输入表单根据 `inputSchema` 自动生成
2. 可切换「表单模式」和「JSON 模式」手动编辑输入
3. 选择执行模式：
   - **SYNC**：同步等待结果返回
   - **ASYNC**：异步提交，不等待结果
4. 查看执行结果（格式化输出）和调试视图（完整上下文、日志、错误详情）
...
1. 点击「上传安装」按钮
2. 选择符合 PF4J 规范的 `.jar` 文件
3. 上传后插件出现在列表中（可能需要手动启动）
...
1. **GitHub 集合**：输入 GitHub 集合 URL，扫描并选择安装
2. **本地目录**：选择本地目录进行安装
3. **归档文件**：上传 `.zip` 归档文件
...
- **模型管理**卡片：显示已启用的模型数量和警告（如缺少 API Key 配置）
- **Agent 管理**卡片：显示已启用的 Agent 数量
- **Toolset 管理**卡片：显示已启用的 Toolset 数量
- **最近运行**表格：最近 8 条 Agent 运行记录
...
1. 确认对话框显示脚本的依赖信息
2. 可选择是否安装关联的依赖项
3. 安装后脚本出现在脚本库中，作用域为 `REPOSITORY`
...
1. 先使用「测试」按钮验证处理器输出（不执行目标脚本）
2. 再使用「试运行」创建一次真实的执行记录
3. 最后在「事件记录」中查看完整的链路信息
...
3. 点击「创建备份」下载 `.zip` 文件（包含 `backup.json` 及 `plugins/`、`skills/` 目录）
...
1. **CLI**：自动展平为 `--name alice` 形式的 flag
2. **Admin UI**：自动生成参数填写表单
3. **AI Agent**：自动理解为 tool description
4. **执行校验**：执行前自动校验入参格式
