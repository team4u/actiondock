...
## 核心区别：Playbook vs. Skill

许多开发者容易混淆 Skill 与 Playbook。它们是两个维度、不同抽象层级的概念，彼此不是替代关系，而是互补协作的关系。

### 1. Skill（技能包）
* **定位**：解决 **“Agent 能不能使用某个底层基础指令”** 的问题。它是 **物理物理级** 的能力分发。
* **分发与安装**：静态文件复制。ActionDock 只是作为“分发介质”，从仓库拉取 Skill 后，**直接将对应的物理文件（如 Claude Skills 的 `.md` 定义文件）写入 IDE 或 AI 客户端的特定本地目录**（例如 `~/.claude/skills`）。
* **运行时归属**：**ActionDock 运行时不负责执行 Skill**。一旦安装完毕，Skill 完全脱离 ActionDock 控制，由 IDE/AI 客户端在本地启动时静态加载并执行。

### 2. Playbook（任务手册）
* **定位**：解决 **“Agent 在特定复杂业务场景下该怎么做”** 的问题。它是 **战术级** 的能力与知识聚合网络。
* **分发与安装**：动态资产挂载。Playbook 是 ActionDock 数据库中管理的结构化模型，随着“能力包（Capability Package）”被安装到具体的项目仓库（Repository）而建立映射关联。
* **运行时归属**：**ActionDock 运行时直接提供 Playbook 服务**。外部 Agent 在运行时通过 ActionDock API/CLI 动态地做意图检索（`resolve`）与导览读取（`guide`）。它是一个集成了工具（Script 引用）、资料（Knowledge 引用）、环境规则与安全边界的综合服务体。

### 维度对比

| 维度 | Skill (技能包) | Playbook (任务手册) |
| :--- | :--- | :--- |
| **物理形态** | 磁盘上的物理 `.md` / `.json` 文件 | 数据库里的结构化资产、动态 API 服务 |
| **解决痛点** | 扩展客户端的单点底层工具箱（如：怎么运行 shell 动作） | 提供复杂场景的专家排查 SOP（如：怎么排查退款超时） |
| **生命周期** | 静态安装到特定物理目录，跟随客户端启动加载 | 动态导入、版本化管理、随能力包（Package）动态挂载仓库 |
| **资源关联** | 单一的底层工具描述文件 | 聚合了 **项目仓库 + Action 脚本 + 文档知识 + 安全边界** 的“一群”资产组合 |
| **运行时主体** | IDE 或 AI 客户端本地运行 | ActionDock 平台提供运行时检索与安全审计 |
...
### Q: 我应该何时编写 Playbook，何时编写 Skill？
* 当你需要**扩展 AI 客户端的基础操作能力**（如：给客户端新增一个可以读写本地特定配置文件的底层工具）时，去写一个 **Skill** 并分发到目标目录。
* 当你需要**将一堆写好的业务脚本（Actions）、业务项目背景（Knowledge）和排查步骤（SOP）聚合成一个特定线上故障的排查能力**，提供给 Agent 消费时，请编写 **Playbook** 并随能力包进行分发。
