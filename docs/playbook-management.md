...
许多开发者容易把 Playbook 和 Skill 混在一起谈。这里说的 Skill，指的是 Claude、Codex 一类 Agent 自身会加载并遵循的原生 Skill 机制，不是 ActionDock 的 Skills 管理或分发能力。两者也不是替代关系：Skill 负责塑造 Agent 的通用行为，Playbook 负责把 Agent 带到某个业务场景里的正确路径上。

### 1. Agent 原生 Skill
* **定位**：扩展 Agent 的通用工作方式、工具使用习惯、流程习惯或角色能力。
* **关注点**：Agent 一般会怎么做，而不是某个仓库里的某个故障怎么排查。
* **作用范围**：通常跨项目复用，不天然绑定某个业务仓库、某组脚本或某批知识文件。
* **运行方式**：由 Agent 或 IDE 客户端自身在会话中加载和遵循。
* **典型例子**：代码评审规范、先读文档再编码、如何调用某类通用工具、固定输出格式。

### 2. Playbook（任务手册）
* **定位**：为某个复杂业务场景提供仓库级、任务级的战术导览。
* **关注点**：在这个业务场景里具体该查什么、读什么、用什么脚本、何时停止。
* **作用范围**：与仓库、知识引用、脚本引用、风险等级、停止条件强绑定。
* **运行方式**：由 ActionDock 在运行时通过 `resolve` 和 `guide` 动态提供。
* **典型例子**：退款失败排查、账单告警诊断、发布回滚检查。

### 维度对比

| 维度 | Agent 原生 Skill | Playbook |
| :--- | :--- | :--- |
| **解决的问题** | 让 Agent 学会一种通用能力或工作习惯 | 告诉 Agent 在特定业务场景里如何行动 |
| **作用范围** | 通常跨项目复用 | 面向特定仓库、业务域和任务场景 |
| **绑定上下文** | 主要依赖 Agent 自身提示与工具能力 | 显式绑定仓库、知识、脚本和风险边界 |
| **运行时角色** | Agent 本地或会话内遵循 | ActionDock 运行时动态检索与提供 |
| **资源关联** | 一般不直接绑定项目内具体资产 | 聚合 `knowledgeRefs`、`scriptRefs`、`stopConditions` 等场景资产 |
| **更新方式** | 更新的是 Agent 的通用行为模板 | 更新的是具体场景的 SOP 与关联资产网络 |
...
* 当你要给 Agent 增加一套**跨项目复用的通用行为约束、操作习惯或工具使用范式**时，写 **Agent 原生 Skill**。常见例子包括代码评审规范、调试流程、输出格式约束、工具调用守则。
* 当你要把某个业务场景下的**脚本、知识、排查路径和风险边界**组织成可被 Agent 动态消费的战术手册时，写 **Playbook**。常见例子包括退款超时排查、计费异常诊断、发布事故处置。
