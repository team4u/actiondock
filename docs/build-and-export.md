...
## 1. 独立二进制构建原理 (`ac build`)

在执行 `ac build` 时，ActionDock 会自动执行以下构建管线：

```text
1. 扫描当前项目的 actions/ 目录与 actiondock.json 配置
                 |
                 v
2. 动态生成静态 Registry 入口 (.actiondock/.build/entry.ts)
...
3. 调用 Bun 原生单文件编译引擎
   ...
   - 将 TypeScript 代码、所有 npm 依赖及 Bun 极速运行时打包进单个可执行文件
                 |
                 v
4. 计算二进制哈希并生成构建元数据 (dist/bin/artifact.json)
```
...
## 2. 交叉编译（Cross-Target Compilation）
...
# 2. 交叉编译为 Linux x86_64 二进制
ac build --target linux-x64

# 3. 交叉编译为 macOS ARM64 (Apple Silicon) 二进制
ac build --target darwin-arm64

# 4. 交叉编译为 Windows x86_64 可执行程序
ac build --target windows-x64
```

---

## 3. 构建元数据规范 (`artifact.json`)
...
## 4. 导出自包含 Skill 交付包 (`ac export skill`)
...
### `SKILL.md` 自动生成规则
导出的 `SKILL.md` 会自动包含：
1. **YAML Frontmatter**：包含 `name` 与 `description`，符合 Antigravity、Claude Code、Cursor 等标准 Skill 规范。
2. **二进制调用说明**：指导 Agent 如何使用 `./bin/<pkg>` 发现工具（`list --json`）、查看参数定义（`describe <id> --json`）与执行 Action（`run <id> --input '...'`）。
3. **Action 目录与参数清单**：自动从每个 Action 的 `inputSchema` 中提取必填与可选参数列表。
4. **Playbook SOP 索引**：列出所有可用的操作手册路径。
