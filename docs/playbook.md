...
Playbook 把选择延后到任务发生之后。Agent 先按项目、意图正则、标签和启用状态搜索候选手册，命中后再读取当前任务需要的指南、知识指针、脚本候选和停止条件。
...
### 3. 最能说服读者的一点：Playbook 是可预演的路由资产

把一个排查场景做成 Skill，或者把手册全文放进知识库，读者很难在任务开始前预演 Agent 会拿到什么。Skill 的边界藏在 Agent 运行环境里，知识库的边界藏在文档正文里；两者都要等 Agent 开始阅读之后，才知道它会不会走对。
