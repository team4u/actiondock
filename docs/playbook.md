...
业务排查场景的基数极其庞大。如果将每个排查场景都打包成常驻 Skill，会导致 Agent 的工具集急剧膨胀。在日常开发、功能编写或普通重构场景下，这些高度特化的排查工具无需被召回。把它们全部载入，会污染运行时上下文、引发工具误用，Agent 执行主流开发任务时的准确率下降，Token 消耗也会上升。

普通的 Agent Skill 属于松散的、无模式约束的提示词或逻辑封装。如果任由开发者随意编写排查类 Skill，产出的质量很难对齐——可能有人只写了一段模糊的自然语言，有人遗漏了核心的风险阻断逻辑，有人写死了特化脚本。

Playbook 则定义了一套标准的手册装配协议。它通过强类型 Schema 约束，迫使手册作者必须在统一的信息模型下思考：必须隔离出风险等级（`riskLevel`）、必须显式定义阻断边界（`stopConditions`）、必须声明证据依赖（`knowledgeRefs`）与候选动作（`scriptRefs`）。有了这套模型，所有排障手册都站在统一的安全底线上，平台还能对契约做静态校验。松散的 Skill 给不了这层保障。
...
中大型项目的知识库庞大，单个仓库内可能包含成百上千篇文档。如果完全依赖知识库，Agent 面对线上故障时，需要先在大批文档里做语义搜索，再逐篇阅读和判断。这种全局扫描很耗时，也容易引入不相关的文档噪声，Token 消耗大，还可能让 Agent 在排障过程中偏离主线。

Playbook 聚焦于“这次任务怎么收窄”。它为特定任务提供一条精简的“导航路径”，直接声明了本次任务必须查阅的少量核心证据（`knowledgeRefs`）、前置停止条件（`stopConditions`）和候选动作。如果把这些边界都埋在项目知识库里，让 Agent 临场去检索和归纳，边界会来得太晚，排障过程也会在文档里来回打转。
...
查询 Schema 是为了确认入参契约：必填字段、枚举值、格式要求和参数来源，这一步先不执行脚本。随后整理临时问题清单：
