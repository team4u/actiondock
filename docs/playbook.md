...
### 1. 为什么不是一个 Playbook 一个 Skill

一个排障场景当然可以写成一个 Agent Skill。比如“退款失败排查”这件事，可以在 Skill 里写清楚：先看退款文档，再查订单日志，必要时调用 `query-refund-log` 脚本，遇到高风险写操作必须停。

第二个、第三个、第五十个排障场景出现后，Agent 启动前就要面对一个选择：全量加载，还是靠用户手动点名。

全量加载会把支付、退款、库存、账单、Webhook 等互不相关的排障规则塞进同一个上下文。手动点名又把路由责任交还给用户，用户必须先知道“这件事应该用哪个 Skill”。

Playbook 把选择延后到任务发生之后。Agent 先按项目、关键词、标签和启用状态搜索候选手册，命中后再读取当前任务需要的指南、知识指针、脚本候选和停止条件。

Skill 适合承载长期能力，Playbook 适合承载一次任务的临时边界。
