...
| 编译与交付模态 | 庞大 Jar 包或依赖后端的 CLI，目标机需部署 JDK/Maven | 单文件独立二进制、Agent Skill、MCP 服务、HTTP 服务 | 支持 ac build 全平台交叉编译，目标机零环境依赖 |
...
| 工作区与环境诊断 | 依赖管理控制台配置能力列表 | ac link 动态扫描、ac info 意图发现与 ac doctor 体检 | 自动感知工作区子包，内置全套环境诊断与健康体检 |
...
- **独立编译交付**：执行 `ac build` 编译为全平台单文件二进制，或通过 `ac export skill` 导出为 Agent Skill。
...
- 单元测试：`bun test`
- 契约校验：`ac action validate <action-id>`
- 环境体检：`ac doctor`
- 本地执行：`ac run <action-id> --input '<json>'`
- 智能体直连：`ac mcp`
- 独立二进制构建：`ac build -t <target>`
- 导出技能包：`ac export skill`
