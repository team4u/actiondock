...
- `ad build`：编译为零外部依赖的单个独立可执行文件（适用于生产部署、CI/CD、容器）。
- `ad export skill`：打包为自包含的 Agent Skill 规范目录（适用于向 Claude Code、Antigravity、Codex 智能体分发）。

---

## 编译为零依赖独立二进制 (`ad build`)

在项目根目录下或使用 `-P <package-id>` 跨目录执行 `ad build`：
...
ad build -P team4u.github-tools
```
...
ad build -t linux-x64 -o ./dist/bin/github-tools-linux

# 为 Linux ARM64 (如 AWS Graviton / 树莓派) 构建
ad build -t linux-arm64 -o ./dist/bin/github-tools-linux-arm64

# 为 macOS Apple Silicon (M 系列芯片) 构建
ad build -t darwin-arm64 -o ./dist/bin/github-tools-macos-arm64

# 为 macOS Intel x86_64 构建
ad build -t darwin-x64 -o ./dist/bin/github-tools-macos-x64

# 为 Windows 平台构建（自动追加 .exe 扩展名）
ad build -t windows-x64 -o ./dist/bin/github-tools-windows
```

---

## 导出为 Agent Skill (`ad export skill`)

`ad export skill` 用于将 Action Package 打包为可供 AI 智能体直接理解与调用的自包含 Skill（支持 `-P` 跨目录导出）。

### 导出源码型 Skill（默认）
```bash
ad export skill
ad export skill -P team4u.github-tools
```
...
- **发布为 GitHub Release 资产**：将 `ad build` 生成的可执行文件或 `ad export skill` 生成的 Skill 压缩包挂载在 Release 资产中。
