...
- 接收 `repo`（仓库全名）与 `prNumber`（PR 编号）。
- 从配置读取 `GITHUB_TOKEN`。
- 调用 GitHub REST API 获取 PR 详情。
...
describe("github.get-pr Action", () => {
  it("可以在内存沙箱中执行与验证", async () => {
    // 创建纯内存测试运行时（无需启动真实数据库或网络服务）
...
- **类型与校验一体**：TypeScript 类型负责编码期推导，JSON Schema 负责运行期校验与 LLM 发现。
- **配置多级回退解析**：`ctx.config` 自动依序回退（调用覆盖 > SQLite 存储 > 环境变量 > 项目默认值 > 代码兜底）。
- **物理通道隔离**：业务数据走 `stdout`，诊断日志走 `stderr`，避免大模型解析报错。
