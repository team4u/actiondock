...
ActionDock 2.0 采用内嵌式 SQLite（基于 Node.js 原生 `node:sqlite`）作为零依赖持久化存储后端，无需安装外部服务。为了彻底解决传统同步数据库驱动阻塞主线程的问题，ActionDock 2.0 在底层引入了基于专用后台工作线程的异步存储驱动，并建立了严格的数据目录租约锁协议与故障自愈机制。

---

## 异步存储引擎：WorkerSqliteDriver

在高并发任务与流式交互场景下，主事件循环必须保持无阻塞运行，以保证协作式取消信号即时分发、流式响应及时传输与定时器按期触发。
