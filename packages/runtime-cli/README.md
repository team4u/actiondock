...
遵循规范的命令行退出状态码与机器契约约定：

- `0`：命令成功执行完毕；在机器输出模式下，意图或关键字搜索无匹配项时同样返回状态码 0 并输出空集合。
- `1`：运行时逻辑异常或业务执行失败（例如远端服务不可达、超时中止或 Action 内部抛出未捕获错误）。
- `2`：命令行参数校验失败、必填选项缺失，或通过 `-P, --package` 指定的目标包/目标目录不存在（严格报错，拒绝向上隐式回退）。

### 通用选项支持

所有下沉运行时命令均支持以下控制选项：

- `-P, --package <id>`：精确定位目标包或路径。
- `--json`：以标准 JSON 格式输出结果。
- `--envelope`：强制包装为标准信封结构对象。
- `--data-dir <path>`：指定自定义数据存储目录。
