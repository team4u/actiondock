# ScriptFlow Plugin Template

ScriptFlow 插件开发模板项目。基于此模板可快速创建自定义插件，扩展 ScriptFlow 的 Groovy 脚本能力。

## 架构概览

ScriptFlow 插件系统基于 [PF4J](https://pf4j.org/) (Plugin Framework for Java) 构建，采用扩展点（Extension Point）模式：

```
scriptflow-plugin-api          插件 API（接口定义 + 工具类）
  └── scriptflow-plugin-template  插件模板（本模块，可直接复制开发）
```

插件作为独立 JAR 包，通过 PF4J 动态加载到 ScriptFlow 主应用中。Groovy 脚本可通过内置的 `plugins` 变量调用已安装插件的功能。
...
确保依赖 `scriptflow-plugin-api`：

```xml
<dependency>
    <groupId>org.team4u</groupId>
    <artifactId>scriptflow-plugin-api</artifactId>
...
### 3. 实现 ScriptFlowPlugin 扩展点

使用 `@Extension` 注解标记实现类，实现 `ScriptFlowPlugin` 接口：
...
@Extension
public class MyScriptFlowPlugin implements ScriptFlowPlugin {
...
在 `src/main/resources/META-INF/scriptflow/plugins/` 下创建 `{pluginId}.json`：

```json
{
  "pluginId": "my-plugin",
  "name": "My Plugin",
  "description": "A custom ScriptFlow plugin.",
  "version": "1.0.0",
  "configSchema": {
    "type": "object",
    "properties": {
...
插件元数据描述，对应 classpath 下的 JSON 文件。用于向 ScriptFlow 主应用声明插件的能力：
...
`configSchema` 使用 JSON Schema 格式描述配置项，ScriptFlow 前端据此自动渲染配置表单：
...
ScriptFlow 在 Groovy 脚本中注入了 `plugins` 变量，可直接调用已安装且已启动的插件：
...
├── pom.xml
└── src/main/
    ├── java/org/team4u/scriptflow/plugin/template/
    │   ├── TemplatePlugin.java          # PF4J Plugin 入口
    │   ├── DemoScriptFlowPlugin.java    # 扩展点实现
    │   └── DemoPluginConfig.java        # 配置类
    └── resources/META-INF/scriptflow/plugins/
        └── scriptflow-demo-plugin.json  # 插件 Manifest
```
...
### scriptflow-plugin-api 模块

| 类/接口 | 说明 |
|---------|------|
| `ScriptFlowPlugin` | 插件扩展点接口，所有插件的核心契约 |
