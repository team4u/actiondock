...
- **日常运行与开发环境**：原生运行于 Node.js 22.12.0 或更高版本。日常的工具开发、单测执行、本地命令行交互、MCP 协议通信以及 HTTP 微服务部署完全依托 Node.js 环境，全面支持 npm、pnpm 与 yarn 包管理器，日常运行完全脱离 Bun。
...
- `@actiondock/cli`：命令行门面工具链，基于 Node.js 22.12.0 或更高版本运行，串联项目脚手架、开发调试、单测运行与打包导出。
