...
class ScheduleControllerSupport {
    ...
    List<ScriptScheduleView> listAll() {
        return scheduleApplicationService.listAll().stream()
                .map(scriptScheduleViewMapper::toView)
                .toList();
    }

    List<ScriptScheduleView> listByScript(String scriptId) {
        return scheduleApplicationService.list(scriptId).stream()
                .map(scriptScheduleViewMapper::toView)
                .toList();
    }

    ScriptScheduleView detail(String scheduleId) {
        return scriptScheduleViewMapper.toView(scheduleApplicationService.getById(scheduleId));
    }

    ApiResponse<ScriptScheduleView> createSchedule(String scriptId, ScriptScheduleUpsertRequest request) {
    ...
    ApiResponse<ScriptScheduleView> updateSchedule(String scriptId, String scheduleId,
