  ...
  return (
    <>
      ...
      <Space direction="vertical" size={16} style={{ width: "100%" }}>
        <PageHeader
          title="工具仓库"
          meta={<Text type="secondary">支持 Git、HTTP 与本地目录仓库。本地目录仓库在创建时会自动初始化为空仓库，不需要手工创建 scriptflow.repository.json，也不用先点同步。</Text>}
          actions={
            <>
      ...
      >
        <Form form={form} layout="vertical" initialValues={{ enabled: true, trustLevel: "UNTRUSTED", type: "GIT" }}>
          <Space direction="vertical" size={12} style={{ width: "100%" }}>
            ...
            >
              <Input placeholder={repositoryType === "LOCAL_DIR" ? "/Users/me/scriptflow-repo" : "https://example.com/repo.git"} />
            </Form.Item>

            {repositoryType === "LOCAL_DIR" ? (
              <Text type="secondary">保存时会自动创建目录、tools/ 子目录和空的 scriptflow.repository.json。</Text>
            ) : null}
