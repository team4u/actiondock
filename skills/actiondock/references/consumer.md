...
本参考手册面向智能体宿主环境，规范技能资产安装、客户端装载路径、Agent 在缺依赖时的按需自举步骤，以及引导智能体正确调度与执行技能的完整生命周期。
...
## Agent 按需自举工序（首次运行失败或缺依赖时）

用户获取或安装技能后，智能体直接调用即可。若首次执行遇到命令不存在（如 `ad: command not found`）或模块缺失报错，智能体在目标环境中按以下步骤自主完成环境自举：
