...
| **执行原子 Action** | `ad run <action> [--input <json> \| --input-file <path\|->] [-r, --raw]` | 简单参数内联传递，复杂对象推荐参数文件；查阅文件或纯文本输出推荐 `-r, --raw` 原始输出 | [cli.md](references/cli.md) |
...
- 文本与正文原始输出：阅读代码、文件内容或将结果作为下游管道输入时，推荐传入 `-r, --raw`。该模式直接将结果正文（如 `content`）原始输出到 stdout（保留真实换行且无 JSON 转义），元数据输出至 stderr，兼顾可读性与精确定位。
