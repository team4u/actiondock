...
- **项目知识库**：解析项目仓库，先读 `PROJECT.md`，再按文档指引检索项目内容 → 读取 `references/project-knowledge.md`
...
| "项目仓库" / "项目知识库" / "PROJECT.md" / "resolve --project" | `references/project-knowledge.md` |
...
- 项目知识入口定位 / `PROJECT.md` / 项目任务文档检索 → 读取 `references/project-knowledge.md`
...
- 项目相关任务必须先解析项目仓库：`actiondock repository resolve --project <repositoryId> --json`。
- 先读 `PROJECT.md`，再按正文里给出的入口文件、目录和关键词搜索；不要一上来就全仓库扫源码。
- 如果 `PROJECT.md` 已明确说不要优先搜索 `dist`、`build`、`node_modules`，就遵守它。
