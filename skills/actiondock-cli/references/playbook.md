...
Playbook 消费完成后，Agent 必须在内部判断是否需要给出“推荐下一步”；但这不是强制输出区。只有当当前证据已经指向后续动作、相关任务、阻断原因或风险边界时，才在用户回复里展示。

判断依据按以下优先级使用：

1. 用户问题、`guideMarkdown`、已读取项目知识、脚本 schema 和脚本执行/查询结果中的事实。
2. `stopConditions`、`riskLevel` 和用户授权状态，用于判断是否应停止、降级为 dry-run 或请求人工确认。
3. `relatedPlaybookRefs`，只作为可选任务导航候选；不得自动递归加载或自动跳转。
4. `scriptRefs`，只作为候选动作池；不得把未证明相关的脚本列入推荐。

有价值时，结果收束按下面结构组织，保持简短：
...
推荐下一步：
- 可继续执行的 Playbook 或低/中风险动作，并说明原因、风险和是否建议立即执行。

当前不建议：
- 不应执行的脚本、修复、重试或跳转，并说明阻止原因。

需要人工确认：
- 涉及生产写操作、补偿、回滚、人工改数、高风险权限或证据不足的事项。
```

输出规则：

- 没有明确下一步时，不要硬凑“推荐下一步”；只说明当前结论和缺口即可。
- 不要把高风险动作和低风险动作混在同一组推荐里；高风险动作必须进入“需要人工确认”或“当前不建议”。
- 不要基于自然语言猜测直接推荐写操作、补偿、退款、回滚、人工改库或批量执行。
- 如果推荐切换 Playbook，必须说明证据如何指向该任务；切换后重新执行五段式消费详情。
- 如果建议继续执行脚本，必须先确认已查询 schema、参数来源完整、风险可接受且未命中停止条件。
