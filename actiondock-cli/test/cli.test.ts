...
beforeAll(async () => {
  server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/api/playbooks/refund-failure/sessions") {
      return json(res, {
        status: 0,
        msg: "ok",
        data: {
          ...
          playbookName: "退款失败排查",
          playbookVersion: "2026-05-02T00:00:00",
          playbookSnapshotHash: "abc123",
          ...
          stopConditionsSnapshot: ["缺少关键上下文"],
          status: "RUNNING",
          currentPhase: "ROUTE",
    ...
    if (req.method === "POST" && req.url === "/api/playbook-sessions/pbs-1/events") {
      return json(res, {
        status: 0,
        msg: "ok",
        data: {
    ...
    if (req.method === "GET" && req.url === "/api/playbook-sessions/pbs-1") {
      return json(res, {
        status: 0,
        msg: "ok",
        data: {
    ...
    if (req.method === "POST" && req.url === "/api/playbook-sessions/pbs-1/complete") {
      return json(res, {
        status: 0,
        msg: "ok",
        data: {
...
describe("CLI integration", () => {
  ...
  it("manages playbook sessions through the cli", async () => {
    requests.length = 0;
    ...
    expect(JSON.parse(start.stdout).id).toBe("pbs-1");
    const startRequest = requests.find((item) => item.method === "POST" && item.url === "/api/playbooks/refund-failure/sessions");
    expect(startRequest?.body).toEqual(expect.objectContaining({
      userPrompt: "rf_123",
      intent: "refund",
      agentName: "cursor",
      agentRunId: "run-1",
      candidatePlaybookIds: ["refund-failure"]
    }));
    ...
    expect(JSON.parse(event.stdout).sequence).toBe(2);
    const eventRequest = requests.find((item) => item.method === "POST" && item.url === "/api/playbook-sessions/pbs-1/events");
    expect(eventRequest?.body).toEqual(expect.objectContaining({
      phase: "BOUND",
      type: "STOP_CONDITION_CHECKED",
      decision: "passed",
      payload: { field: "refundId" }
    }));

    const timeline = await runCli(["playbook", "session", "show", "pbs-1", "--timeline", "--server", baseUrl]);
    expect(timeline.status).toBe(0);
    expect(timeline.stdout).toContain("[ROUTE] SESSION_STARTED started playbook:refund-failure");
    expect(timeline.stdout).toContain("[BOUND] STOP_CONDITION_CHECKED passed - 只读查询");
    ...
    expect(JSON.parse(complete.stdout)).toEqual(expect.objectContaining({
      id: "pbs-1",
      status: "COMPLETED",
      finalSummary: "done"
    }));
