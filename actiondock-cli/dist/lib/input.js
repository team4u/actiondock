...
export function collectDynamicFlags(argv, options) {
...
export function parseInputObject(inputJson, inputFile, labels = {
    jsonFlag: "`--input-json`",
    fileFlag: "`--input-file`"
}) {
...
export function parseJsonValueInput(inputJson, inputFile, labels) {
    return parseJsonInput(inputJson, inputFile, labels);
}
function parseJsonInput(inputJson, inputFile, labels) {
...
export function buildInputFromSchema(baseInput, dynamicFlags, fields, labels = {
    ...
    if (missingRequired.length > 0) {
        throw new ActionDockCliError(`缺少必填参数: ${missingRequired.map((field) => `--${field.name}`).join(", ")}`, 2, missingRequired.map((field) => ({ field: field.name, kind: field.kind })));
    }
...
function resolveJsonSourceName(inputJson, inputFile, labels) {
    if (inputJson) {
...
function fallbackValue(rawValue) {
    ...
    switch (field.kind) {
        ...
        case "enum":
            if (!field.enumValues.includes(rawValue)) {
                throw new ActionDockCliError(`字段 ${field.name} 只能是以下值之一: ${field.enumValues.join(", ")}`, 2);
            }
            return rawValue;
        default:
            return rawValue;
