...
describe("applyProcessorFieldOverrides", () => {
    it("clears event trigger processor fields when patch uses empty objects", () => {
        ...
        const merged = applyProcessorFieldOverrides(mergeDefinitionPatch(existing, patch), patch, ["filterProcessor", "idempotencyProcessor", "inputProcessor"]);
    ...
    it("preserves normal merge behavior for non-empty processor patches", () => {
        ...
        const merged = applyProcessorFieldOverrides(mergeDefinitionPatch(existing, patch), patch, ["normalizationProcessor"]);
