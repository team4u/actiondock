 ...
 */
export async function startHttp(ctx, opts) {
    if (!LOOPBACK_HOSTS.has(opts.host)) {
        throw new Error(`Refusing to bind HTTP transport to non-loopback host '${opts.host}'. ` +
    ...
    server.listen(opts.port, opts.host, () => {
        console.error(`[actiondock-mcp] http transport ready: http://${opts.host}:${opts.port}${opts.endpoint}`);
    });
 ...
 */
async function handleRequest(req, res, ctx, opts) {
