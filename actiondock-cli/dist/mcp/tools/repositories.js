 ...
 * </ul>
 */
export function registerRepositoryTools(server, ctx) {
    ...
    registerActionDockTool(server, {
        ...
        description: "Resolve a project repository and return its ACTIONDOCK.md content (full text in the `content` field).",
        risk: "read",
        inputSchema: {
        ...
        policy,
        client,
        handler: async (args) => client.repositories.resolveProject(args.repositoryId)
    });
    // ─── actiondock_repository_script_list ───────────────────
    registerActionDockTool(server, {
        name: "actiondock_repository_script_list",
        description: "List scripts available in a repository. Omit repositoryId to list across all repositories.",
        risk: "read",
        inputSchema: {
            repositoryId: z
                .string()
        ...
        policy,
        client,
        handler: async (args) => client.repositories.listScripts(args.repositoryId)
    });
    ...
    registerActionDockTool(server, {
        ...
        policy,
        client,
        handler: async (args) => client.repositories.getScript(args.repositoryId, args.scriptId)
    });
    // ─── actiondock_repository_knowledge_list ────────────────
    registerActionDockTool(server, {
        name: "actiondock_repository_knowledge_list",
        description: "List knowledge entries available in a repository. Omit repositoryId to list across all repositories.",
        risk: "read",
        inputSchema: {
            repositoryId: z
                .string()
        ...
        policy,
        client,
        handler: async (args) => client.repositories.listKnowledge(args.repositoryId)
    });
    ...
    registerActionDockTool(server, {
        ...
        policy,
        client,
        handler: async (args) => client.repositories.getKnowledge(args.repositoryId, args.knowledgeId)
    });
}
