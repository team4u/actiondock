...
export async function runServiceAction(action: ServiceAction, args: string[]): Promise<number> {
  ...
  throw new ActionDockCliError(
    "ActionDock service management is currently supported on macOS and Linux only. Use `actiondock server` on this platform.",
    2,
  );
}

async function runMacServiceAction(action: ServiceAction, args: string[]): Promise<number> {
...
async function runLinuxServiceAction(action: ServiceAction, args: string[]): Promise<number> {
...
function macPlist(args: string[], logsDir: string): string {
...
function linuxUnit(args: string[]): string {
...
async function run(command: string, args: string[], options: { allowFailure?: boolean } = {}): Promise<number> {
