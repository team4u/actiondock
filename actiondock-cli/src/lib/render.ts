...
export function renderWebhookDetail(item: WebhookDefinition): string {
  const lines = [
    `Webhook: ${item.id}`,
    `Key: ${item.key ?? "-"}`,
    `Name: ${item.name ?? "-"}`,
...
export function renderRepositoryWebhookList(items: RepositoryWebhookDescriptor[]): string {
  if (items.length === 0) {
    return "没有仓库Webhook。";
  }
  return items
...
export function renderRepositoryWebhookDetail(item: RepositoryWebhookDetail): string {
...
export function renderRepositoryLocalAsset(item: RepositoryLocalAsset): string {
  const lines = [
...
export function renderRepositoryList(items: RepositoryDefinition[]): string {
  if (items.length === 0) {
    return "没有仓库。";
  }
...
export function renderRepositoryDetail(item: RepositoryDefinition): string {
  const lines = [
    `Repository: ${item.id}`,
    `Name: ${item.name}`,
    `Type: ${item.type}`,
...
export function renderWebhookInvokeResult(result: WebhookInvokeResult): string {
  const lines = [
    `HTTP ${result.status}`
  ];
