...
export function renderRepositoryEventSourceList(items: RepositoryEventSourceDescriptor[]): string {
  if (items.length === 0) {
    return "没有仓库事件源。";
  }
  return items
...
export function renderRepositoryEventSourceDetail(item: RepositoryEventSourceDetail): string {
