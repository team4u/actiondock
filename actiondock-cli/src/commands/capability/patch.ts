...
export default class CapabilityPatchCommand extends BaseCommand {
  static description = "Apply a JSON Merge Patch to an ActionDock capability draft binding";
  ...
  static flags = {
    ...
    source: Flags.string({
      description: "Replace capability source inline"
    }),
    "source-file": Flags.string({
      description: "Replace capability source using a text file"
    }),
  ...
  async run(): Promise<void> {
    const { args, flags } = await this.parse(CapabilityPatchCommand);
    ...
    try {
      ...
      const item = await client.patchCapability(args.capabilityId, patch);
