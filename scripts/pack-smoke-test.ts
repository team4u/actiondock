...
try {
  for (const pkg of packages) {
    ...
    const tgzFilename = tgzMatch ? tgzMatch[0] : `actiondock-${pkg}-2.0.2.tgz`;
...
`;
  ...
  if (verProc.exitCode !== 0 || !verProc.stdout.toString().includes("2.0.2")) {
    throw new Error(`'ad --version' failed: ${verProc.stderr.toString()} (output: ${verProc.stdout.toString()})`);
  }
  console.log(`   ✓ ad --version returned 2.0.2`);
